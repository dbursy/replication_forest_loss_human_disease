**********************************************
* Comments to original replication do file
...
* set matsize 10000
 ...
 use "C:\LocalData\fiestas-flores\Seafile\Personal\Replication Games\Replication\Data\dta.dta", clear
...
*** Comment ***
		* Can't open this link: http://userforum.dhsprogram.com/index.php?t=getfile&id=4 
		**** used this: https://userforum.dhsprogram.com/index.php?t=msg&goto=9556&&reveal=121, seems similar
		* This is also not working: https://esa.un.org/unpd/wpp/Download/Standard/ASCII/
		**** used this https://population.un.org/dataportal/data/indicators/41/locations/566/start/2008/end/2013/table/pivotbylocation?df=3493b84e-4815-4950-ac97-68b5850e9ec
		

** Regressions from Table 1: 
...
estout m1_fever m2_fever m3_fever m1_diarrhea m2_diarrhea m3_diarrhea m1_cough m2_cough m3_cough using "analysis_.xls", ///
cells(b(fmt(%7,3f)) se(abs fmt(%7,3f)) t(abs fmt(%7,3f))) stats(N r2, fmt(%17,3f)) keep(forest_* _cons) replace  


*** Conclusion: It does replicata WITH ANALYSIS DATA
...
*** Robustness 0: Alt. Denormalization with analysis data ***
...
gen var = DN_wgt - DN_wgt_c // The corrected weight is larger, that is, larger population estimates (bigger s.e.) paper understimated?
...
* change to . or , depending on the system and country
cd "C:\LocalData\fiestas-flores\Seafile\Personal\Replication Games\Replication\Tables"
estout m1_fever m2_fever m3_fever m1_diarrhea m2_diarrhea m3_diarrhea m1_cough m2_cough m3_cough using "analysis_altnd.xls", ///
cells(b(fmt(%7,3f)) se(abs fmt(%7,3f)) t(abs fmt(%7,3f))) stats(N r2, fmt(%17,3f)) keep(forest_* _cons) replace  
		
*** Conclusion: Using the correct de-normalization provides similar restuls as the one provided in the paper
	
	
*** Robustness 3: Avoid potential overspecification problems ***
...
* change to . or , depending on the system and country
cd "C:\LocalData\fiestas-flores\Seafile\Personal\Replication Games\Replication\Tables"
estout m3_1_fever m3_2_fever m3_3_fever m3_1_diarrhea m3_2_diarrhea m3_3_diarrhea m3_1_cough m3_2_cough m3_3_cough using "analysis_over.xls", ///
cells(b(fmt(%7,3f)) se(abs fmt(%7,3f)) t(abs fmt(%7,3f))) stats(N r2, fmt(%17,3f)) keep(forest_* _cons) replace


*** Conclusion: Using the correct de-normalization and a more parsimonious model we fin that
